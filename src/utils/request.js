    ...
    */
    ...
    _success = function(that, _config, res, resolve, reject) {
        if (res.statusCode >= 200 && res.statusCode <= 302) { // http ok
            ...
            if (that.interceptor.response && typeof that.interceptor.response === 'function') {
              result = that.interceptor.response(result, _config)
            }
            if (result.success || result.constructor === ArrayBuffer) { // 接口调用业务成功
              const _data = _config.business ? result[_config.business] : result;
              if (_config.debug) {
    ...
    _fail = function(that, _config, res, resolve, reject) {
        let result
        _config.debug || console.error('response failure: ', res)
